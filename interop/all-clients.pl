...
sub get_data
{
    ...
    my @lines = `perl -Mblib interop/clients/$name.pl`;
    chomp for @lines;
...
my @rtrlib = get_data("rtrlib");
my $rtrlib_version = $rtrlib[0]->[1];
...
my @table;
push @table, ["", "rtrlib ($rtrlib_version)",
              "rtrtr ($rtrtr_version)",
...
if (@utls != 1) {
    use Data::Dumper;
    warn Dumper(\@rtrlib, \@rtrtr, \@rpki_rtr_demo);
    die "tests should produce same table lengths";
}

for (my $i = 0; $i < @rtrlib; $i++) {
    my $line1   = $rtrlib[$i];
    my $name1   = $line1->[2];
    my $result1 = $line1->[3];
