...

{
    ...
    # Limit the client to IPv4 PDUs.

    my $client =
        APNIC::RPKI::RTR::Client->new(
            server     => '127.0.0.1',
            port       => $port,
            data_types => [PDU_IPV4_PREFIX()]
        );
    ...
    # Refresh the client, and confirm that only the IPv4 PDUs are
    # received.
    ...
    # Add another IPv6 PDU.  Refresh the client, confirm the serial
    # number is incremented, and confirm that the PDU count stays the
    # same.

    my $client_serial = $client->state()->serial_number();
    ...
    my $new_client_serial = $client->state()->serial_number();
    isnt($new_client_serial, $client_serial,
        "Serial updated even though data PDU not returned");

    # Confirm resubscription is accepted if a no-op, and rejected
    # otherwise.

    $client->_init_socket_if_not_exists();
    $client->_send_subscribing_data_pdu();
    eval { $client->refresh(1) };
    $error = $@;
    ok((not $error), 'Got successful response from server');
    if ($error) {
    ...
    $client->_init_socket_if_not_exists();
    $client->{'data_types'} = [PDU_IPV6_PREFIX()];
    $client->_send_subscribing_data_pdu();
    eval { $client->refresh(1) };
    $error = $@;
    ok($error, 'Failed to refresh on resubscription');

    # Confirm error on subscribing for non-data PDU.

    $client->{'data_types'} = [PDU_SERIAL_NOTIFY()];
    $client->_init_socket_if_not_exists();
    my ($res, undef, $other_pdu) =
        $client->_process_responses(1);
    ok($other_pdu, 'Got back other PDU');
    is($other_pdu->type(), PDU_ERROR_REPORT(),
        'Other PDU is error PDU');
